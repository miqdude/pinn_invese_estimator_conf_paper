...
\begin{enumerate}
    \item \textbf{Rank-Free PINN Estimation:} We develop an in-the-loop PINN that dynamically identifies the unknown payload shift by utilizing a short-term receding horizon removing dependency of highly-variant persistent excitation.
    \item \textbf{Singularity-Free Manifold Optimization:} The network’s loss function embeds energy-based translational Euler-Lagrange and rotational Euler-Poincaré equations directly on the $SO(3)$. 
    \item \textbf{Dynamic Control Integration:} The continuously updated offset parameters are fed directly into the control allocation matrix to immediately restore the tracking stability.
\end{enumerate}
